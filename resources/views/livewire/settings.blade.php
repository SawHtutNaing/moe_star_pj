<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl p-6 transition-all duration-300">
            <h2 class="text-2xl font-semibold text-gray-800 mb-6 sm:text-3xl">Settings Management</h2>
            ...
            @if ($isOpen)
                <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 px-4 sm:px-0">
                    <div class="bg-white p-6 rounded-xl w-full max-w-lg shadow-2xl animate-slide-up">
                        <h3 class="text-xl font-semibold text-gray-800 mb-6">{{ $setting_id ? 'Edit Setting' : 'Add Setting' }}</h3>
                        <form wire:submit.prevent="store">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model="value" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200 text-sm">
                ...
                <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 sm:px-6 text-left text-sm font-medium text-gray-700 tracking-wide">Value</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($settings as $setting)
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                <td class="px-4 py-4 sm:px-6 text-sm text-gray-900">{{ $setting->name }}</td>
                                <td class="px-4 py-4 sm:px-6 text-sm text-gray-900">{{ $setting->value }}</td>
                                <td class="px-4 py-4 sm:px-6 flex gap-2 flex-wrap">
                                    <button wire:click="edit({{ $setting->id }})" class="bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-1 px-3 rounded-lg transition-colors duration-200 text-xs">Edit</button>
                                    <button wire:click="delete({{ $setting->id }})" class="bg-red-500 hover:bg-red-600 text-white font-medium py-1 px-3 rounded-lg transition-colors duration-200 text-xs">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
