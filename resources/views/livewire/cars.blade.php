<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl p-6 transition-all duration-300">
            <h2 class="text-2xl font-semibold text-gray-800 mb-6 sm:text-3xl">Car Management</h2>
            ...
            @if ($isOpen)
                <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 px-4 sm:px-0">
                    <div class="bg-white p-6 rounded-xl w-full max-w-lg shadow-2xl animate-slide-up">
                        <h3 class="text-xl font-semibold text-gray-800 mb-6">{{ $car_id ? 'Edit Car' : 'Add Car' }}</h3>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($cars as $car)
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                <td class="px-4 py-4 sm:px-6 text-sm text-gray-900">{{ $car->name }}</td>
                                <td class="px-4 py-4 sm:px-6">
                                    @if ($car->image)
                                        <img src="{{ asset('storage/' . $car->image) }}" alt="{{ $car->name }}" class="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg">
                                    @else
                                        <span class="text-sm text-gray-500">No Image</span>
                                    @endif
                                </td>
                                <td class="px-4 py-4 sm:px-6 flex gap-2 flex-wrap">
                                    <button wire:click="edit({{ $car->id }})" class="bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-1 px-3 rounded-lg transition-colors duration-200 text-xs">Edit</button>
                                    <button wire:click="delete({{ $car->id }})" class="bg-red-500 hover:bg-red-600 text-white font-medium py-1 px-3 rounded-lg transition-colors duration-200 text-xs">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
